...
MODE_CSS = {
    "MTR": "mode-mtr",
    "Citybus": "mode-citybus",
    "Green Minibus": "mode-gmb",
    "HKU Shuttle Bus": "mode-shuttle",
...
with col_panel:
    ...
    st.markdown(
        '<p class="section-title">🗺️ Plan Your Route</p>', unsafe_allow_html=True
    )
    ...
    if routes:
        ...
        st.markdown(
            '<div class="empty-state"><div class="empty-icon">🗺️</div><p>No routes found.</p></div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            '<div class="empty-state"><div class="empty-icon">🚌</div><p>Select origin and destination, then press <b>Find Routes</b>.</p></div>',
            unsafe_allow_html=True,
        )
